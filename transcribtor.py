...
def transcribtor(word):
    ...
    word = re.sub(r'ъ([яёюеЯЁЮЕ])',
                  lambda m: "й" + {'Я': 'А', 'Ё': 'О', 'Ю': 'У', 'Е': 'Э', 'я': 'а', 'ё': 'о', 'ю': 'у', 'е': 'э'}[
                      m.group(1)], word)
    word = re.sub(r'([аоиыэуйАОИЫЭУяёюеЯЁЮЕ])([яёюеЯЁЮЕ])', lambda m: m.group(1) + "й" +
                                                                      {'Я': 'А', 'Ё': 'О', 'Ю': 'У', 'Е': 'Э', 'я': 'а',
    ...
    word = re.sub(r"([жшц]+)'", r"\1", word)

    word = re.sub(r"([аоиыэуАОИЫЭУ])'([аоиыэуАОИЫЭУ])", r"\1\2", word)
    ...
    if re.findall(r'[аоиыэу]', word):
        word = re.sub(r"(['йч])([оаиэ])", r'\1ь', word)
        word = re.sub(r"(['йч])([у])", r'\1ь°', word)
        word = re.sub(r"([^'йч\b])([оаыэ])", r'\1ъ', word)
        word = re.sub(r"([^'йч\b])([у])", r'\1ъ°', word)
        word = re.sub("о", "а", word)
        word = re.sub("э", "и", word)
